// Debug printing, assertion helpers, and tensor-dump functions.
//
// ValueInfo wraps the Q15 scale factor needed to convert int16_t elements
// back to human-readable floats for printing.  Constructed from a
// ParameterInfo* so callers don't have to extract scale manually.
//
// dump_matrix / dump_params / dump_params_nhwc
//   Pretty-print tensors from NVM for visual inspection.  They support both
//   Q15 float-formatted output and raw integer output (controlled by the
//   dump_integer global).  When USE_PROTOBUF is defined the values are also
//   written to model_output_data for offline comparison with a reference
//   (e.g., PyTorch or ONNX Runtime).
//
// check_ofm_dumped
//   Uses a per-node OFM_DUMPED flag in NodeFlags to print each layer's
//   output exactly once across power cycles — even if the test harness
//   re-executes the same node after recovery.  The flag is toggled on each
//   dump and compared against the run_counter parity.
//
// compare_vm_nvm_impl
...
//   ParameterInfo boundaries in the intermediate-parameters area.
//
// dump_footprints
//   Prints both shadow copies of the HAWAII footprint for a layer so that
//   recovery correctness can be verified manually.
...
#define PRINT_NEWLINE_IF_DATA_NOT_SAVED \
  if (!layer_out) {                     \
    my_printf(NEWLINE);                 \
  }
...
static void print_q15(LayerOutput* layer_out, int16_t val,
...
static void dump_params_common(Model* model, const ParameterInfo* cur_param,
  ...
  my_printf(NEWLINE);
  my_printf("Channel last: %s" NEWLINE,
            cur_param->param_flags & CHANNEL_LAST ? "true" : "false");
...
static void extract_dimensions(const ParameterInfo* cur_param, uint16_t* NUM,
                               uint16_t* H, uint16_t* W, uint16_t* CHANNEL) {
  if (cur_param->dims[3]) {
    // 4-D tensor, NCHW
...
static bool check_ofm_dumped(const ParameterInfo* cur_param) {
  ...
  uint8_t has_ofm_dumped_flag = static_cast<uint8_t>(
      (node_flags->general_flags & OFM_DUMPED) == OFM_DUMPED);
...
void dump_params_nhwc(Model* model, const ParameterInfo* cur_param,
  ...
  for (uint16_t n = 0; n < NUM; n++) {
    ...
    for (uint16_t tile_c_base = 0; tile_c_base < CHANNEL;
         tile_c_base += output_tile_c) {
        ...
        for (uint16_t h = 0; h < H; h++) {
          for (uint16_t w = 0; w < W; w++) {
            ...
            if (cur_param->param_flags & TRANSPOSED) {
              ...
              offset2 = n * W * H * CHANNEL + W * H * tile_c_base +
                        w * H * cur_tile_c + h * cur_tile_c + c;
            } else {
              // internal format is NHWC
              offset2 = n * H * W * CHANNEL + H * W * tile_c_base +
                        h * W * cur_tile_c + w * cur_tile_c + c;
            }
...
void compare_vm_nvm_impl(int16_t* vm_data, Model* model,
  ...
  for (uint16_t offset = 0; offset < blockSize; offset += BUFFER_TEMP_SIZE) {
    ...
    my_memcpy_from_param(model, buffer_temp, output, output_offset + offset,
                         cur_block_size * sizeof(int16_t));
...
void check_nvm_write_address_impl(uint32_t nvm_offset, size_t n) {
  if (nvm_offset >= INTERMEDIATE_PARAMETERS_INFO_OFFSET &&
      nvm_offset < MODEL_OFFSET) {
    MY_ASSERT((nvm_offset - INTERMEDIATE_PARAMETERS_INFO_OFFSET) %
                  sizeof(ParameterInfo) ==
              0);
  } else if (nvm_offset < INTERMEDIATE_PARAMETERS_INFO_OFFSET) {
    MY_ASSERT(n <= INTERMEDIATE_PARAMETERS_INFO_OFFSET - nvm_offset,
              "Size %d too large!!! nvm_offset=%d" NEWLINE, n, nvm_offset);
  }
}
...
void dump_footprints(uint16_t layer_idx) {
  ...
  for (uint8_t copy_id = 0; copy_id < 2; copy_id++) {
    ...
    read_from_nvm(&tmp_footprint, nvm_addr<Footprint>(copy_id, layer_idx),
                  sizeof(Footprint));

    if (copy_id == 0) {
      uint8_t cached_copy_id = *copy_id_cache_addr<Footprint>();
      my_printf_debug("Footprint NVM pointer=%d, cached pointer=%d" NEWLINE,
                      tmp_footprint.version, cached_copy_id);
      MY_ASSERT(cached_copy_id == 0 ||
                cached_copy_id == tmp_footprint.version + 1);
    }
