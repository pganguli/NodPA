"""
Measure intermittent-inference performance on a physical device over UART.

Reads lines from the MSP430/MSP432 UART output, parses 'I' (inference) and
'R' (recharge) timing messages, and accumulates statistics over a sliding
window of the last N inferences.  Prints a running summary with:
  - average wall time including recharging pauses
  - average active-computation time (excluding recharging)
  - average number of power-failure events per inference

Usage: python exp/measure-intermittent.py [--port /dev/ttyACM0] [--verbose]
"""
...
class DataHandler:
    ...
    def report(data, prefix=""):
        ...
        print(
            f"[{datetime.datetime.now()}] {prefix}With recharging: {with_recharging:.4f}, without recharging: {without_recharging:.4f}, power failures: {power_failures:.1f}"
        )
    ...
    def feed(self, data):
        ...
        if cmd == "I":
            ...
            if with_recharging < self.recharging:
                ...
                warnings.warn(
                    f"Incorrect recharging time! {with_recharging} < {self.recharging}"
                )
                self.recharging = 0
                self.power_failures = 0
                return
            ...
            if len(self.last_n) > self.N:
                self.last_n.pop(0)
            self.report(
                np.mean(self.last_n, axis=0),
                prefix=f"Average of last {len(self.last_n)}: ",
            )
