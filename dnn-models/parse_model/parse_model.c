/*
 * ONNX binary inspector: reads a .onnx file from stdin and prints a human-
 * readable summary of all graph nodes, tensor shapes, and parameter sizes.
 *
 * Useful for debugging transform.py output or understanding a new model
 * before porting it.  The binary is linked against protobuf-c and the
 * auto-generated onnx.proto3.pb-c.c decoder.
 *
 * Usage: ./parse_model < model.onnx
 *
 * handle_value_info: prints name, shape, and estimated memory footprint for
 *   one tensor descriptor (ValueInfoProto).  batch_size is tracked but not
 *   counted in mem_usage since it is dynamic.
 *
 * handle_node: prints the op_type, input names, output names, and all
 *   attributes for one graph node.
 *
 * main: reads the entire protobuf from stdin into a heap buffer (up to
 *   MAX_MSG_SIZE bytes), unpacks it with onnx__model_proto__unpack, then
 *   iterates over graph inputs, outputs, initializers, and nodes.
 */
...
static void handle_quantization_annotation(Onnx__TensorAnnotation* annotation) {
  ...
  for (i = 0; i < annotation->n_quant_parameter_tensor_names; i++) {
    Onnx__StringStringEntryProto* entry =
        annotation->quant_parameter_tensor_names[i];
