"""
Dataset loaders for CIFAR-10, Google Speech Commands (KWS), and UCI HAR.

Each loader downloads and caches its dataset on first call, applies the
preprocessing required by the corresponding ONNX model, and returns a ModelData
named-tuple with the PyTorch Dataset and data-layout metadata.
"""
...
def load_data_cifar10(
    train: bool, target_size: tuple[int, int], normalized: bool = False
) -> ModelData:
    ...
    transforms.append(
        Resize(size=target_size[-2:], antialias=True)
    )  # H and W from NCHW of ONNX
    ...
    return ModelData(
        dataset=dataset, data_layout=DataLayout.NCHW, label_names=dataset.classes
    )
...
def load_data_google_speech(
    train: bool, target_size: tuple[int, int], for_cnn: bool = False
) -> ModelData:
    xdg_cache_home = platformdirs.user_cache_path()
    ...
    dataset = TensorDataset(
        torch.from_numpy(mfccs.astype(np.float32)), torch.tensor(labels)
    )
...
def load_har(train: bool, target_size: tuple[int, int]):
    try:
        ...
        archive_dir = download_file(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/00240/UCI%20HAR%20Dataset.zip",
            filename="UCI HAR Dataset.zip",
            post_processor=functools.partial(extract_archive, subdir="UCI HAR Dataset"),
        )
        ...
        dataset = TensorDataset(
            torch.from_numpy(X.astype(np.float32)), torch.from_numpy(labels - 1)
        )
