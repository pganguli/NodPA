/******************************************************************************
*
* Linker command file for the MSP430FR5962 on the Riotee board, used by the
* NodPA intermittent-inference firmware.
*
* Derived from TI's stock MSP430FR5962 linker file (as shipped with the Riotee
* dummy project) with two NodPA-specific additions taken from the 5994 build:
*
*   1. The 8 KB SRAM is split exactly as on the 5994: the lower 4 KB
*      (0x1C00-0x2C00) stays general-purpose RAM, and the upper 4 KB
*      (0x2C00-0x3C00) is carved into LEARAM + LEASTACK for the Low-Energy
*      Accelerator (the FR5962 has the same LEA as the FR5994).
*
*   2. .nvm / .nvm2 NOINIT sections (FRAM-resident, retained across resets) and
*      .leaRAM / .leaStack placement.
*
* FRAM note: the FR5962 has only 128 KB internal FRAM (FRAM 48 KB + FRAM2
* ~80 KB) versus 256 KB on the 5994.  Code (.text) and constant model tables
* (.const) are spilled across both banks via the large code/data model.  The
* multi-megabyte model weights / activations live in the EXTERNAL SPI FRAM, not
* here.
*
*****************************************************************************/
...
MEMORY
{
    ...
    /* Lower half of SRAM: general-purpose RAM (CPU-exclusive). */
    RAM                     : origin = 0x1C00, length = 0x1000
    /* Upper half of SRAM: shared with the LEA. */
...
SECTIONS
{
    ...
    /* ISR code (incl. _c_int00_noargs startup) must land in lower 64KB so the
     * 16-bit reset vector reaches it by zero-extension.  Use strict > FRAM (not
     * >>) so this placement is committed before the split-placed .const can fill
     * FRAM.  Must appear before .const in SECTIONS so the allocator processes it
     * first. */
    .text:_isr        : {}  > FRAM          /* Code ISRs incl. _c_int00_noargs   */

    /* NodPA fills FRAM bank 1 with .const/.text, so let the init/constructor
     * tables spill into FRAM2 (they don't fit in bank 1 alone). */
    .cinit            : {}  >> FRAM | FRAM2 /* Initialization tables             */
    .binit            : {}  >> FRAM | FRAM2 /* Boot-time Initialization tables   */
    .pinit            : {}  >> FRAM | FRAM2 /* C++ Constructor tables            */
    .init_array       : {}  >> FRAM | FRAM2 /* C++ Constructor tables            */
    .mspabi.exidx     : {}  >> FRAM | FRAM2 /* C++ Constructor tables            */
    .mspabi.extab     : {}  >> FRAM | FRAM2 /* C++ Constructor tables            */
    ...
    .text             : {} > FRAM           /* Code                              */
#else
    .text             : {} >> FRAM | FRAM2  /* Code; startup must be in lower 64KB for 16-bit reset vector */
#endif
    ...
    .nvm        : {} > FRAM  type = NOINIT  /* NodPA NVM-resident (retained)     */
    .nvm2       : {} > FRAM2 type = NOINIT  /* NodPA NVM-resident (retained)     */
    .stack      : {} > RAM (HIGH)           /* Software system stack             */

    .tinyram    : {} > TINYRAM              /* Tiny RAM                          */

    .leaRAM     : {} > LEARAM   type = NOINIT  /* LEA working buffer (lea_buffer) */
